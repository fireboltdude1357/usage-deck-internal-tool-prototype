...
| `src/routes/+page.server.ts`, `src/routes/{platform,market,provisioned,success,adoption}-*/+page.ts`, `src/lib/selection.svelte.ts`, `src/lib/refresh.svelte.ts` | `docs/architecture.md` § Frontend, `docs/data-flow.md` § Request path. |
| `src/routes/adoption-engagement/*`, `buildAdoptionEngagementSnapshot` in `aggregator.ts`/`roster.ts`, `runAdoptionEngagementPipeline` in `pipeline.ts` | `docs/architecture.md` § Schema (AdoptionEngagement). Definitions are load-bearing: adopter = any user seen in the picker window (cumulative within window); engaged in month M = ≥1 session in trailing 3 months [M-2, M] (rolling, users can drop out and re-engage). |
...
| `src/lib/behavior-graph/*`, `src/lib/server/posthog/behavior-graph-*.ts`, `src/routes/behavior-graph/*`, `src/routes/api/posthog/behavior-graph/+server.ts` | `docs/architecture.md` § Behavior graph. The graph + sessions are computed live from PostHog page-load events (not snapshotted), so no schema update needed. |
